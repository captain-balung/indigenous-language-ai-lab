const categories = [
  { id: "basics", title: "基礎學習", short: "打好基礎", description: "從聲音、字母與詞彙開始，一步步建立族語能力。", icon: "🌱", accent: "green" },
  { id: "classroom", title: "課堂測驗", short: "測測實力", description: "用有趣的小挑戰，即時檢視每一堂課的學習成果。", icon: "📝", accent: "blue" },
  { id: "certification", title: "認證模擬", short: "考前演練", description: "熟悉認證題型與節奏，為正式挑戰做好萬全準備。", icon: "🏆", accent: "gold" },
  { id: "scenarios", title: "情境應用", short: "生活開口說", description: "走進真實生活場景，練習把族語自然地說出來。", icon: "🏕️", accent: "coral" },
  { id: "interaction", title: "學習互動", short: "邊玩邊學", description: "透過遊戲、故事與互動，把學習變成一場冒險。", icon: "🎮", accent: "purple" }
];

const applicationSeeds = {
  basics: [
    ["聲音探險家", "聆聽並辨識族語的聲音與節奏。", "🎧"],
    ["字母練習室", "認識字母與發音，建立閱讀基礎。", "🔤"],
    ["詞彙收藏冊", "用主題卡片累積生活常用詞彙。", "📚"],
    ["跟讀小教練", "跟著提示反覆練習，勇敢開口說。", "🗣️"]
  ],
  classroom: [
    ["聽力快問快答", "聽見關鍵詞，選出最合適的答案。", "👂"],
    ["詞語配對賽", "將族語詞彙與圖片正確配對。", "🧩"],
    ["句子排列所", "重新排列詞語，拼出完整句子。", "🧱"],
    ["課堂挑戰榜", "完成一組綜合題目，確認學習進度。", "📊"]
  ],
  certification: [
    ["初級模擬站", "以模擬題熟悉初級認證的答題方式。", "🥉"],
    ["中級模擬站", "挑戰進階聽讀題型與作答節奏。", "🥈"],
    ["口說練習官", "依題目提示組織內容並練習表達。", "🎤"],
    ["考前任務包", "集中演練多種題型，準備上場。", "🎒"]
  ],
  scenarios: [
    ["部落的一天", "在日常情境中練習問候與對話。", "🏠"],
    ["市場小幫手", "學會購物、數量與食物相關說法。", "🧺"],
    ["旅行會話包", "從問路到搭車，練習實用句型。", "🚌"],
    ["文化故事屋", "跟著情境故事理解語言與文化。", "🔥"]
  ],
  interaction: [
    ["族語闖關島", "完成關卡、收集徽章，展開學習冒險。", "🗺️"],
    ["故事共創機", "選擇角色與情節，一起完成族語故事。", "📖"],
    ["對話小夥伴", "透過安全的引導情境練習生活對話。", "🤖"],
    ["節奏記憶王", "跟著節拍記住詞語與常用句型。", "🥁"]
  ]
};
  ...
  return `
    <section class="category-section category-section--${category.accent}" id="${category.id}" aria-labelledby="${category.id}-title">
      <div class="category-heading">
        <div class="category-heading__icon" aria-hidden="true">${category.icon}</div>
        <div>
...
function renderCard(application, index, accent) {
  ...
  return `
    <${tagName} class="app-card app-card--${accent}${isAvailable ? " app-card--available" : ""}" ${linkAttributes}>
      ...
      <div class="app-card__icon" aria-hidden="true">${application.icon}</div>
